...
#words - the data
#n_words - number of words to keep track of, more than this unique words will be converted to the
#unknown symbol
...
class Word2Vec(object):
  ...
  def __init__(self):
    self.data_index = 0
    self.batch_size = 128
    self.embedding_size = 256  # Dimension of the embedding vector.
    ...
    self.token_size = 2000
    self.num_steps = 10001


  def build_dataset(self, words):
    """Process raw inputs into a dataset."""
    n_words = self.token_size
    count = [[0, -1]]
  ...
  def train_skipgram(self, data, vocabulary_size, reverse_dictionary, sym_dict, mem_offset):
    
    batch_size = self.batch_size
    embedding_size = self.embedding_size  # Dimension of the embedding vector.
    skip_window = self.skip_window       # How many words to consider left and right.
    num_skips = self.num_skips         # How many times to reuse an input to generate a label.
    num_sampled = self.num_sampled      # Number of negative examples to sample.
