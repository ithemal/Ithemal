...
class DataInstructionEmbedding(DataCost):
    ...
    def single_pass_extract_data(self, cnx, fmt, columns, arch, progress=True):
        self.fields = ['code_id'] + columns

        sql = (
            ' SELECT code.code_token, code.code_id, code.code_intel, AVG(times.time) ' +
            ' FROM code ' +
            ' JOIN times ON code.code_id = times.code_id ' +
            " WHERE times.arch=2 AND times.kind='actual' " +
            ' GROUP BY code_id '
        )

        data = ut.execute_query(cnx, sql, True)

        def extract_text(value):
            return list(map(int, filter(bool, value.split(','))))

        if fmt == 'text':
            extractor = extract_text
        elif fmt == 'bin':
            raise NotImplementedError('fmt = "bin" is not supported yet')
        else:
            raise ValueError('fmt must be one of "text" or "bin" (got "{}")'.format(fmt))
        ...
        self.raw_data = list(prog_func(map(
            lambda x: tuple([extractor(x[0])] + list(x[1:])),
            data
        )))
